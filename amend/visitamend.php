...
$visit_data = null;
if ($visit_id > 0) {
    $visit_query = $conn->prepare("SELECT * FROM visitmast WHERE VISSNO = ?");
    $visit_query->bind_param("i", $visit_id);
    $visit_query->execute();
    $visit_data = $visit_query->get_result()->fetch_assoc();
...
if ($batchResult) {
    while ($row = $batchResult->fetch_assoc()) {
        $batches[$row['BATFARSNO']][] = [
            ...
            'sno' => $row['BATSNO'],
            'date' => $row['BATDDT'],
            'chicks' => $row['BATCHICKS'], // Initial chick count
            'current_birds' => $row['batblnbrd'], // Current balance birds
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $visblnbird = $batchDetails['batblnbrd'] + $visit_data['VISMORTALITY'] - $vismortality; // Adjust bird count
    
    // Calculate feed balance
    $visfeedconsumed = $visinpfeedbag - $visfeedbal;

    // Calculate FCR (Feed Conversion Ratio)
    $total_weight = $visavgwgt * $visblnbird;
    $total_feed_consumed_kg = $visfeedconsumed * 50;
    $visfcr = ($total_weight > 0) ? ($total_feed_consumed_kg / $total_weight) : 0;
    $visavgfeed = $total_feed_consumed_kg / $visblnbird;

    // Set user and IP information
    $visamduser = $current_user['USRNAME'];
    $visamdip = $_SERVER['REMOTE_ADDR'];
    $visusrsno = $user_id;
    ...
    try {
        ...
        $stmt->bind_param(
            "iiisididdddddsssisssi",
            $visfarsno, $visbatsno, $visfieldoff, $visddt, $visage,
            $vismortality, $vismotpcn, $visblnbird, $visavgwgt,
            $visavgfeed, $visfcr, $visinpfeedbag, $visfeedconsumed,
            $visfeedbal, $visamduser, $visamdip, $visusrsno,
            $viscgcon, $visgenremarks, $visremarks, $visit_id
        );
        $stmt->execute();
        $stmt->close();

        // Update batch bird count
        $update_batch_sql = "UPDATE batmast SET batblnbrd = ? WHERE batsno = ?";
        $update_batch_stmt = $conn->prepare($update_batch_sql);
        $update_batch_stmt->bind_param("ii", $visblnbird, $visbatsno);
        ...
        $conn->rollback();
        echo "<script>
                alert('Error updating visit: " . addslashes($e->getMessage()) . "');
                window.history.back();
              </script>";
        exit;
    }
...
<head>
    ...
    <title><?php echo $visit_id > 0 ? 'Edit' : 'Add'; ?> Visit Record</title>
    ...
    <script>
        $(document).ready(function() {
            ...
            const visitData = <?php echo json_encode($visit_data); ?>;

            // If editing, populate form fields
            if (visitData) {
                $('#FARMER_ID').val(visitData.VISFARSNO).trigger('change');
                setTimeout(() => {
                    $('#VISBATCODE').val(getBatchCodeFromSno(visitData.VITBATSNO));
                    $('#VISFIELDOFF').val(visitData.VISFIELDOFF);
                    $('#VISDDT').val(visitData.VISDDT);
                    $('#VISMORTALITY').val(visitData.VISMORTALITY);
                    $('#VISAVGWGT').val(visitData.VISAVGWGT);
                    $('#VISINPFEEDBAG').val(visitData.VISINPFEEDBAG);
                    $('#VISFEEDBAL').val(visitData.VISFEEDBAL);
                    $('#VISCGCON').val(visitData.VISCGCON || '');
                    $('#VISREMARKS').val(visitData.VISREMARKS || '');
                    $('#VISGENREMARKS').val(visitData.VISGENREMARKS || '');
                    
                    // Trigger calculation after all fields are set
                    calculateFields();
                }, 100);
            }

            // Helper function to find batch code by sno
            function getBatchCodeFromSno(batSno) {
                for (const farmerId in batchesByFarmer) {
                    for (const batch of batchesByFarmer[farmerId]) {
                        if (batch.sno == batSno) {
                            return batch.code;
                        }
                    }
                    ...
                    batchesByFarmer[farmerId].forEach(function(batch) {
                        $('#VISBATCODE').append(
                            `<option value="${batch.code}" 
                             ...
                             data-current-birds="${batch.current_birds}"
            ...
            function calculateFields() {
                ...
                if (batchCode && visitDate) {
                    ...
                    const currentBirds = parseFloat(selectedBatch.data('current-birds')) || 0;
                    ...
                    try {
                        ...
                        // Calculate balance birds based on current batch count and mortality change
                        const originalMortality = visitData ? visitData.VISMORTALITY : 0;
                        const balanceBirds = currentBirds + originalMortality - mortality;
...
<body>
    <div class="form-container">
        <form method="POST" action="">
            <h1><?php echo $visit_id > 0 ? 'Edit' : 'Add'; ?> Visit Record</h1>
                ...
                <div class="form-column">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="date" id="VISDDT" name="VISDDT" required>
                        </div>
                        <div class="form-group">
